...
router.post("/addcomments", fetchuser, async (req, res) => {
  try {
    let response;
    try {
      response = await axios.post("http://127.0.0/get_comments", req.body);
    ...
    if (filteredExistingUrls.length > 0) {
      ...
      const deleteCommentPromises = filteredExistingUrls.map(
        async (existingUrl) => {
      ...
      const urlModelUpdatePromises = filteredExistingUrls.map(
        async (existingUrl) => {
